...
def run_ssh_command(
    vm: str, zone: str, command: str, is_tpu: bool = False
...
def create_dashboard(
    gpu_status: str, tpu_status: str, progress_group: Progress
) -> Table:
    ...
    grid.add_row(
        Panel(
            gpu_status, title="🔥 NVIDIA A100 (Legacy)", border_style="green", height=12
        ),
        Panel(
            tpu_status,
            title="⚡ Google TPU v5e (Target)",
            border_style="blue",
            height=12,
        ),
    )
    ...
    container.add_row(
        Panel(progress_group, title="⏱️ Simulation Progress", border_style="yellow")
    )
...
def cleanup() -> None:
    ...
    def nuke_gpu() -> None:
        check = subprocess.run(
            f"gcloud compute instances describe {GPU_VM} --project={PROJECT} --zone={GPU_ZONE}",
            shell=True,
        ...
        subprocess.run(
            f"gcloud compute instances delete {GPU_VM} --project={PROJECT} --zone={GPU_ZONE} --quiet",
            shell=True,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
    ...
    def nuke_tpu() -> None:
        check = subprocess.run(
            f"gcloud compute tpus tpu-vm describe {TPU_VM} --project={PROJECT} --zone={TPU_ZONE}",
            shell=True,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        ...
        subprocess.run(
            f"gcloud compute tpus tpu-vm delete {TPU_VM} --project={PROJECT} --zone={TPU_ZONE} --quiet",
            shell=True,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
...
def run() -> None:
    ...
    header("🚀 Accelerator Comparison: CUDA vs TPU")

    # --- SCENE 1: The Context & Conversion ---
    step(
        "Context: The 'Legacy' Training Loop",
        """
    ...
    gpu_p_task = provisioning_progress.add_task(
        "[green]Nvidia A100 VM", total=100, status="Starting..."
    )
    tpu_p_task = provisioning_progress.add_task(
        "[blue]Google TPU VM", total=100, status="Starting..."
    )
    ...
    def run_with_retry(cmd: str, task_id: Any, task_name: str) -> bool:
        ...
        for attempt in range(1, max_retries + 1):
            if attempt > 1:
                provisioning_progress.update(
                    task_id, status=f"Retrying ({attempt}/{max_retries})..."
                )
                time.sleep(5)
    ...
    def do_gpu() -> None:
        ...
        if check.returncode != 0:
            ...
            if not run_with_retry(gpu_create_cmd, gpu_p_task, "GPU"):
                provisioning_progress.update(
                    gpu_p_task, completed=100, status="[bold red]FAILED[/bold red]"
                )
                return

        provisioning_results["gpu"] = True
        provisioning_progress.update(
            gpu_p_task, completed=100, status="[bold green]ONLINE[/bold green]"
        )
    ...
    def do_tpu() -> None:
        ...
        if check.returncode != 0:
            ...
            if not run_with_retry(tpu_create_cmd, tpu_p_task, "TPU"):
                provisioning_progress.update(
                    tpu_p_task, completed=100, status="[bold red]FAILED[/bold red]"
                )
                return

        provisioning_results["tpu"] = True
        provisioning_progress.update(
            tpu_p_task, completed=100, status="[bold green]ONLINE[/bold green]"
        )
    ...
    with console.status("[bold yellow]Waiting for SSH connectivity (Booting)..."):
        if not wait_for_ssh(GPU_VM, GPU_ZONE, is_tpu=False):
            console.print(
                f"[bold red]Error:[/bold red] {GPU_VM} failed to boot or is unreachable."
            )
            return
        if not wait_for_ssh(TPU_VM, TPU_ZONE, is_tpu=True):
            console.print(
                f"[bold red]Error:[/bold red] {TPU_VM} failed to boot or is unreachable."
            )
            return
    ...
    with console.status("[bold green]Syncing files..."):
        if not LEGACY_PAYLOAD.exists() or not JAX_PAYLOAD.exists():
            console.print(
                f"[bold red]Error: Payload files not found at {PAYLOAD_DIR}[/bold red]"
            )
            return

        res_gpu = subprocess.run(
            f"gcloud compute scp {LEGACY_PAYLOAD} {GPU_VM}:~/ --project={PROJECT} --zone={GPU_ZONE} --quiet",
            shell=True,
        )
        if res_gpu.returncode != 0:
            console.print(
                f"[bold red]Error:[/bold red] Failed to SCP payload to {GPU_VM}."
            )
            return

        res_tpu = subprocess.run(
            f"gcloud compute tpus tpu-vm scp {JAX_PAYLOAD} {TPU_VM}:~/ --project={PROJECT} --zone={TPU_ZONE} --quiet",
            shell=True,
        )
        if res_tpu.returncode != 0:
            console.print(
                f"[bold red]Error:[/bold red] Failed to SCP payload to {TPU_VM}."
            )
            return

    console.print("[green]✅ Codebase Deployed to Cloud Layers.[/green]")
    console.input("\n[dim]Press Enter to START THE COMPARISON...[/dim]")

    # --- SCENE 4: The Comparison ---
    header("🏁 The Benchmark: ResNet-50 Comparison")
    gpu_log: list[str] = []
    tpu_log: list[str] = []
    progress = Progress(
        SpinnerColumn(),
    ...
    gpu_task = progress.add_task(
        "[green]ResNet-50 (A100)", total=300, info="Initializing..."
    )
    tpu_task = progress.add_task(
        "[blue]ResNet-50 (TPU)", total=300, info="Initializing..."
    )
            ...
            if "Step" in line:
                try:
                    ...
                    if match:
                        ...
                        progress.update(
                            gpu_task, completed=step_num, info=f"Step {step_num}/300"
                        )
                except Exception:
                    pass
            gpu_log.append(line)
            ...
            if "Step" in line:
                try:
                    ...
                    if match:
                        step_num = int(match.group(1))
                        progress.update(
                            tpu_task, completed=step_num, info=f"Step {step_num}/300"
                        )
                except Exception:
                    pass
            tpu_log.append(line)
